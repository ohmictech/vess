...
**Minting creates value.** A memory-hard VM grinds through a 256 MiB scratchpad until it finds a nonce that satisfies a STARK-provable difficulty target. The resulting `VessBill` carries an unforgeable proof of computation tied to a denomination and owner. Difficulty is hardcoded — no adjustment, no inflation schedule, no halvings.
...
**Identity is invisible.** Dual-Key Stealth Addressing (DKSAP) with ML-KEM-768 means every payment targets a unique one-time address. View tags let the recipient scan traffic in constant time. Human-readable `+tags` (e.g. `+alice`) map to stealth addresses through a DHT so the user never handles a raw public key.
...
# Register and confirm a tag
vess register-tag alice
vess confirm-tag alice --mint-id <HEX>

# Look up a tag
vess lookup-tag alice

# Back up the wallet
vess backup ~/vess-backup.json
...
**Tag hardening.** Tags start unhardened (30-day TTL) after Argon2id registration. A proof-of-payment "confirms" the tag permanently.
...
```bash
cargo test --workspace  # 184 tests, 0 warnings
```
