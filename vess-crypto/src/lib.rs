...
pub fn check_difficulty(hash: &[u8; 32], target_bits: u32) -> bool {
    ...
    for i in 0..full_bytes { if hash[i] != 0 { return false; } }
    if rem_bits > 0 { hash[full_bytes] >> (8 - rem_bits) == 0 } else { true }
}

pub fn block_reward(diff_bits: u32) -> Amount {
    if diff_bits >= 32 { return 1u64 << 32; }
    1u64 << diff_bits
}
...
#[cfg(feature = "full")]
fn arr_bytes(a: &impl AsRef<[u8]>) -> Vec<u8> { a.as_ref().to_vec() }
...
pub fn argon2id_key(password: &[u8], salt: &[u8]) -> [u8; 32] {
    use argon2::Argon2;
    let a = Argon2::new(argon2::Algorithm::Argon2id, argon2::Version::V0x13,
...
#[cfg(feature = "full")] pub const DSA_PUBKEY_BYTES: usize = 1952;
#[cfg(feature = "full")] pub const DSA_SEED_BYTES: usize = 32;
#[cfg(feature = "full")] pub const DSA_SIGNATURE_BYTES: usize = 3309;
...
#[cfg(feature = "full")]
pub fn dsa_pubkey_hash(pk: &[u8]) -> [u8; 32] { blake3_hash(pk) }
...
pub fn dsa_verify(pk: &[u8], msg: &[u8], sig: &[u8]) -> bool {
    ...
    let (Ok(vk), Ok(sig)) = (vk, DsaSignature::<MlDsa65>::try_from(sig))
        else { return false; };
    vk.verify(msg, &sig).is_ok()
}

#[cfg(feature = "full")]
pub fn node_id(pk: &[u8]) -> [u8; 32] { blake3_hash(pk) }

// ML-KEM-512

#[cfg(feature = "full")] pub const KEM_PUBKEY_BYTES: usize = 800;
#[cfg(feature = "full")] pub const KEM_SEED_BYTES: usize = 64;
#[cfg(feature = "full")] pub const KEM_CIPHERTEXT_BYTES: usize = 768;
#[cfg(feature = "full")] pub const KEM_SHARED_SECRET_BYTES: usize = 32;
#[cfg(feature = "full")] pub const KEM_PK_BYTES: usize = 800;
#[cfg(feature = "full")] pub const KEM_CT_BYTES: usize = 768;
...
mod tests {
    ...
    fn test_blake3_multi() {
        assert_eq!(blake3_hash_multi(&[b"ab", b"c"]), blake3_hash_multi(&[b"a", b"bc"]));
    }
